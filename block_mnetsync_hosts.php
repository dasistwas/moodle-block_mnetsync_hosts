...
class block_mnetsync_hosts extends block_list {
    function init() {
        $this->title = get_string('blocktitle','block_mnetsync_hosts');
    }
    ...
    function get_content() {
        ...
        if (is_mnet_remote_user($USER)) {
            if (debugging() and !empty($CFG->debugdisplay)) {
                $this->content = new stdClass();
                $this->content->footer = html_writer::tag('span',
                    get_string('error_localusersonly', 'block_mnetsync_hosts'),
                    array('class' => 'error'));
                return $this->content;
            } else {
                return '';
        ...
        if (!is_enabled_auth('mnetsync')) {
            if (debugging() and !empty($CFG->debugdisplay)) {
                $this->content = new stdClass();
                $this->content->footer = html_writer::tag('span',
                    get_string('error_authmnetneeded', 'block_mnetsync_hosts'),
                    array('class' => 'error'));
                return $this->content;
            } else {
                return '';
        ...
        if (!has_capability('moodle/site:mnetlogintoremote', context_system::instance(), NULL, false)) {
            if (debugging() and !empty($CFG->debugdisplay)) {
                $this->content = new stdClass();
                $this->content->footer = html_writer::tag('span',
                    get_string('error_roamcapabilityneeded', 'block_mnetsync_hosts'),
                    array('class' => 'error'));
                return $this->content;
            } else {
                return '';
        ...
        if ($hosts) {
            foreach ($hosts as $host) {
                $icon  = '<img src="'.$OUTPUT->pix_url('i/moodle_host') . '"'.
                         ' class="icon" alt="'.get_string('server', 'block_mnetsync_hosts').'" />&nbsp;';
                ...
                if ($host->id == $USER->mnethostid) {
                    ...
                    $this->content->items[]="<a title=\"" .s($host->name).
                        "\" href=\"{$CFG->wwwroot}/auth/mnetsync/jump.php?hostid={$host->id}\">" .$icon. s($host->name) ."</a>";
                }
            }
        }
